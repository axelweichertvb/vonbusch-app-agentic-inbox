...
export function useComposeForm(mailboxId?: string, _folder?: string) {
	...
	const formTitle = useMemo(() => {
		if (isDraftEdit) return "Entwurf bearbeiten";
		switch (composeOptions.mode) { case "reply": return "Antworten"; case "reply-all": return "Allen antworten"; case "forward": return "Weiterleiten"; default: return "Neue Nachricht"; }
	...
	const handleSaveDraft = async () => {
		...
		try {
			...
			toastManager.add({ title: "Entwurf gespeichert!" });
		}
		catch (err: unknown) {
			const message = (err instanceof Error ? err.message : null) || "Entwurf konnte nicht gespeichert werden.";
			setError(message);
	...
	const handleSend = async (e: FormEvent, onClose: () => void) => {
		...
		if (!currentMailbox || !mailboxId) { setError("Kein Postfach ausgewählt."); return; }
		const toRecipients = splitEmailList(to);
		if (toRecipients.length === 0) { setError("Fügen Sie mindestens einen Empfänger hinzu."); return; }
		...
		setIsSending(true); toastManager.add({ title: "E-Mail wird gesendet..." });
		try {
			...
			toastManager.add({ title: "E-Mail gesendet!" });
			onClose();
		} catch (err: unknown) { const message = (err instanceof Error ? err.message : null) || "E-Mail konnte nicht gesendet werden."; setError(message); toastManager.add({ title: message, variant: "error" }); }
